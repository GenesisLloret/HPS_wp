    ...
    <div class="wrap">
        ...
        foreach ($mods as $mod) {
            ...
            <div class="module-settings">
                <h3><?php echo esc_html($module_settings['mod_name']); ?>
                    (v<?php echo esc_html($module_settings['mod_version']); ?>)</h3>
                ...
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    ...
                    <?php if ($module_settings['mod_WSForm_moderation']): ?>
                        <input type="text" name="wsform_id"
                            value="<?php echo esc_attr(get_option('hps_wsform_id_' . sanitize_title($module_settings['mod_name']), '')); ?>"
                            placeholder="<?php esc_attr_e('Enter WSForm ID', 'hps_wp'); ?>" />
                    <?php endif; ?>
                    ...
                    <?php if (in_array('activate', $module_settings['mod_has_options'])): ?>
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                            ...
                            <input type="hidden" name="module_name" value="<?php echo esc_attr($module_settings['mod_name']); ?>">
                            <button type="submit" class="button"><?php esc_html_e('Activate', 'hps_wp'); ?></button>
                        </form>
                    <?php endif; ?>
                </form>
            </div>
